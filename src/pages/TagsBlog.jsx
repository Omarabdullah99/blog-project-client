  ...
  return (
    ...
    {tagsBlog &&
        ...
        <div key={blog?._id} className="p-3 shadow-lg rounded cursor-pointer text-black h-[450px] mt-3">
         ...
         <Link to={`/userblog/${blog?.creator}`}> <p className="mb-2 text-sm text-gray-500"><FaUser className="inline-flex items-center mr-2" /> {blog?.name}(see other blogs this user)</p></Link> 
        </div>
      ))}
  </div> 
  )
